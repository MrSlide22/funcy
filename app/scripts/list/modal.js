        ...
        window.service.anuncio.getAnuncio(id)
            .then(function(anuncio) {
                ...
                return new Promise(function(res, rej) {
                    ...
                    //main image
                    var mainImg = '<picture>';
                    mainImg += '<source srcset="' + anuncio.images.xs[0] + '" media="(max-width: 768px)">';
                    mainImg += '<source srcset="' + anuncio.images.md[0] + '"  media="(min-width: 769px) and (max-width: 1200px)">';
                    ...
                    for (var i = 0; i < anuncio.images.xs.length; i++) {
                        ...
                        var img = '<picture>';
                        img += '<source srcset="' + anuncio.images.xs[i] + '" media="(max-width: 768px)">';
                        img += '<source srcset="' + anuncio.images.md[i] + '"  media="(min-width: 769px) and (max-width: 1200px)">';
        ...
        window.service.anuncio.getRelatedAnuncios()
            .then(function(anuncios) {
                ...
                for (var i = 0; i < anuncios.length; i++) {
                    ...
                    relates += '<picture>';
                    relates += '<source srcset="' + anuncios[i].images.xs[0] + '" media="(max-width: 768px)">';
                    relates += '<source srcset="' + anuncios[i].images.md[0] + '" media="(min-width: 769px) and (max-width: 1200px)">';
                    ...
                    relates += '<span class="label label-primary pull-right anuncio-price">$' + anuncios[i].price + '</span>';
                    relates += '</div>';
                    relates += '</div>';
                    relates += '</div>';
                    relates += '</div>';
